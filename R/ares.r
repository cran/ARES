...
"aresCalc" <-
function(rawdata, bootsize=NULL, maxsize=NULL){
...
if(size >= maxsize)
  ...
  if (is.infinite(richness)|richness>1e6){richness=1e6}
  ...
  for (i in 1:bootsize) estimat[, i] = aftbino(size, total = nvec[i], Q = Qlist[[i]], sizes = 1:maxsize)[,2]
...
"aresPlot" <-
function(output_aresCalc,T = NULL){
...
"fmtbino" <- 
function(count=NULL,x=NULL,size=NULL){
  if(is.null(count)){
...
"rpldtbino" <-
function(Q,size,to.conditional=FALSE) {
  x=1-(1-Q[,1])^size
  if(to.conditional)
    Q[,2]=Q[,2]*x
  else
...
"emtbino" <-
function(dens,G=1,Q=NULL,control=list()) {
...
"vtbino" <-
function(dens, Q=NULL, control=list()) {
...
"batbino" <-
function(dens,Q,total=NULL,control=list()){
  ctrl=list(eeps=1e-06,emax=5000)
  ctrl[names(control)]=control
  k=nrow(Q)
  Q=list(Q)
  ...
  if(!is.null(total)){
    size=length(dens)
    attr(Q,"loglik")=total*sapply(Q,function(D)
          sum(dens*log(dmtbino(x=1:size,size=size,Q=D))))
  }
  return(Q)
}

"aftbino" <-
function(size,total,Q,sizes=NULL) {
  Q=mtbino(Q,span=0)
  if(is.null(sizes))
    sizes=1:(4*size)
  if(Q[1,1]>0){
...
"aftbino.mbe" <-
function(count,estimated.richness=NULL,conf.level=0.95) {
  size=length(count)
  if (is.null(estimated.richness))
    estimated.richness=ifelse(all(count[1:2]>0),
      count[1]^2/count[2]/2/size*(size - 1)+sum(count), Inf)
  posi=(1:size)[count>0]
  count=count[count>0]
